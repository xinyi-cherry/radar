...
def cal_RD(file_num, file_name, adcData, num_ADCSamples = 128, num_chirps = 255, num_frames = 32):
    ...
    num_V = int(pad_chirps/2) # VT图纵轴长度
    #-----------------------------------

    TH_cfar = np.zeros((pad_chirps, num_ADCSamples, num_v_frames), dtype=complex)
    data_RD = np.zeros((num_V, num_R, num_v_frames), dtype=complex)
    RT = np.zeros((num_R, num_v_frames), dtype=complex)
    VT = np.zeros((num_V, num_v_frames), dtype=complex)
    ...
    # ----------------------------------------输出图像----------------------------------------
    # plt.figure(figsize=(num_v_frames, num_R), dpi=1)
    # plt.imshow(abs(RT), cmap='gray')
    # plt.axis('off')
    ...
    # plt.savefig('output/'+file_name+'_RT_cross4_pad'+str(file_num)+'.jpg', transparent=True, dpi=1, pad_inches=0)
    # print(file_name + ' RT complete')
    ...
    plt.savefig('output/'+file_name+'_VT_cross4_pad'+str(file_num)+'.jpg', transparent=True, dpi=1, pad_inches=0)
    print(file_name + ' VT complete')
    

if __name__ == '__main__':
    file_name = 'slope180_chirp255_apple_Raw_0'
