...
def get_attn_subsequence_mask(seq):
    ...
    return subsequence_mask.cuda() # [batch_size, tgt_len, tgt_len]
...
class Decoder(nn.Module):
    def __init__(self,length):
        ...
        self.tgt_emb = nn.Embedding(length, d_model)
        self.pos_emb = PositionalEncoding(d_model)
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        dec_enc_attn_mask = get_attn_pad_mask(dec_inputs, enc_inputs).cuda() # [batc_size, tgt_len, src_len]
...
class Transformer(nn.Module):
    def __init__(self,length):
        ...
        self.decoder = Decoder(length).cuda()
        self.dnn = DNNModel().cuda()
        self.projection = nn.Linear(d_model, length, bias=False).cuda()
    def forward(self, enc_inputs, dec_inputs):
        ...
        enc_input = self.dnn(enc_inputs)
        enc_input = torch.transpose(enc_input,0,1)
